...
pub mod noise {
    ...
    impl<S: AsyncRead + AsyncWrite + Unpin> EncryptedTcpStream<S> {
        pub async fn send(&mut self, buf: &[u8]) -> Result<(), HandshakeError> {
            // Encrypt the data with the current nonce
            let encrypted_data = self.encrypt(buf)?;
            ...
            self.inner.write_all(&msg_len_buf).await?;
        ...
        async fn read_data_from_stream(&mut self) -> Result<(), HandshakeError> {
            ...
            if matches!(self.read_state, ReadState::ReadingLength) {
                ...
                self.inner.read_exact(&mut msg_len_buf).await?;
            ...
            while bytes_read < target_length {
                ...
                let read_result =
                    timeout(READ_ITERATION_TIMEOUT, self.inner.read(read_slice)).await??;
        ...
        fn decrypt_received_data(&mut self) -> Result<Vec<u8>, HandshakeError> {
            ...
            self.decrypt(encrypted_data, &nonce).map_err(|e| e.into())
        }
        ...
        pub async fn recv(&mut self) -> Result<Vec<u8>, HandshakeError> {
            ...
            match result {
                Ok(res) => res,
                Err(_) => Err(HandshakeError::from(std::io::Error::new(
    ...
    /// - `Err(HandshakeError)`: If there was an error during generation.
    pub fn generate_keypair() -> Result<Keypair, HandshakeError> {
    ...
    pub fn diffie_hellman(
        my_private_key: &[u8],
        their_public_key: &[u8],
    ) -> Result<RistrettoPoint, HandshakeError> {
        ...
        let their_public = CompressedRistretto::from_slice(their_public_key)?
            .decompress()
            .ok_or_else(|| {
                HandshakeError::from(std::io::Error::new(
    ...
    pub fn decode_shared_secret(encoded: &str) -> Result<Vec<u8>, HandshakeError> {
    ...
    mod tests {
        ...
        fn test_decode_shared_secret() {
            ...
            let decoded =
                decode_shared_secret(encoded).expect("Failed to decode the shared secret");
            let expected =
        ...
        fn test_encrypt_decrypt() {
            ...
            let decrypted_data =
                decrypt(secret, &encrypted_data, &nonce).expect("Decryption failed");
